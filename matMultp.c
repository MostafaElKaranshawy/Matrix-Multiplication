...
void write_to_file(int matrix[20][20], int n, int m, int method, char path[]) {
...
int main() {
    ...
    FILE *aFile;
    char path[100] = "/home/moka/Desktop/OS/Lab2/test3/";
    char a_path[100] = "";
    strcat(a_path, path);
    strcat(a_path, "a.txt");
    
    aFile = fopen(a_path, "r");
    get_size(aFile);
    n1 = rows, m1 = cols;
    ...
    strcat(b_path, path);
    strcat(b_path, "b.txt");
    bFile = fopen(b_path, "r");
    rows = 0, cols = 0;
    get_size(bFile);
    n2 = rows, m2 = cols;

    // Reading Matrix b.
    bFile = fopen("test1/b.txt", "r");
